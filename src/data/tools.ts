...
export const tools: Tool[] = [
  ...
  },
  {
    ...
    name: "Preference alignment (RLHF/RLAIF)",
    category: "Alignment workflow",
    stage: "Alignment",
    status: "Course-covered",
    course: "Class7",
    page: "content/Tool_wiki/tools/class7-alignment.md",
    tags: ["alignment", "rlhf", "preference", "ppo", "dpo", "grpo"],
    itemType: "Workflow stack",
    role: "Shape model behavior with chosen/rejected preferences after SFT",
    abstractionLevel: "Workflow stack: preference data plus alignment method",
    learningLevel: "Intermediate",
    stack: "Class7 alignment stack",
    learnerNote: {
      zh: "Class7 是 SFT 之後的偏好對齊；用同一份 preference data 比較 PPO、DPO、GRPO 的取捨。",
      en: "Class7 is post-SFT preference alignment; it compares PPO, DPO, and GRPO trade-offs on the same preference data.",
    },
    zh: {
      summary: "用人類或 AI 的 chosen/rejected 偏好，把 SFT 後的模型行為調整到更符合期望。",
      useWhen: "模型任務已能完成，但語氣、安全、偏好或推理風格還需要進一步對齊。",
      avoidWhen: "尚未完成 SFT、沒有一致的偏好標註，或只需要 prompt 調整就能解決。",
      notes: "記錄 preference data 來源與標註規則，並用同一份 eval prompts 比較不同方法。",
    },
    en: {
      summary: "Uses human or AI chosen/rejected preferences to tune post-SFT model behavior toward what people want.",
      useWhen: "Use when the model can already do the task but tone, safety, preference, or reasoning style needs alignment.",
      avoidWhen: "Avoid before SFT is done, without consistent preference labels, or when prompt tweaks already suffice.",
      notes: "Record preference data sources and labeling rules, and compare methods on the same eval prompts.",
    },
  },
  {
    id: "ppo",
    name: "PPO (Proximal Policy Optimization)",
    category: "Alignment method",
    stage: "Alignment",
    status: "Course-covered",
    course: "Class7",
    page: "content/Tool_wiki/tools/class7-alignment.md",
    tags: ["ppo", "rlhf", "reward-model", "reinforcement-learning", "alignment"],
    itemType: "Method",
    role: "Classic RLHF: preference to reward model to RL",
    abstractionLevel: "Alignment training method",
    learningLevel: "Advanced",
    stack: "Class7 alignment stack",
    learnerNote: {
      zh: "PPO 是 ChatGPT 式 RLHF；控制力強但需要 reward model，且容易不穩定與 reward hacking。",
      en: "PPO is ChatGPT-style RLHF; powerful control but needs a reward model and risks instability and reward hacking.",
    },
    zh: {
      summary: "三段式 RLHF：先用偏好訓 reward model，再用 RL 在 reward signal 下微調 policy。",
      useWhen: "安全關鍵或需要對行為做細粒度控制，且能負擔 reward model 與 RL 的複雜度。",
      avoidWhen: "資源有限、團隊不熟 RL，或 DPO 已能達到目標時。",
      notes: "監控 reward hacking：reward 升高不代表行為變好，要用 held-out eval 驗證。",
    },
    en: {
      summary: "Three-stage RLHF: train a reward model from preferences, then fine-tune the policy with RL under that reward.",
      useWhen: "Use for safety-critical or fine-grained behavior control when you can afford reward-model and RL complexity.",
      avoidWhen: "Avoid with limited resources, RL-unfamiliar teams, or when DPO already meets the goal.",
      notes: "Watch for reward hacking: rising reward is not better behavior—validate with a held-out eval.",
    },
  },
  {
    id: "dpo",
    ...
    course: "Class7",
    page: "content/Tool_wiki/tools/class7-alignment.md",
    tags: ["dpo", "preference", "alignment", "stable", "no-reward-model"],
    itemType: "Method",
    role: "Direct preference optimization without a separate reward model",
    abstractionLevel: "Alignment training method",
    learningLevel: "Intermediate",
    stack: "Class7 alignment stack",
    learnerNote: {
      zh: "DPO 直接用 chosen/rejected pair 做 classification-style loss，簡單穩定，常是對齊首選。",
      en: "DPO uses chosen/rejected pairs in a classification-style loss—simple and stable, often the default choice.",
    },
    zh: {
      summary: "跳過 reward model 與 RL，直接在 chosen/rejected pair 上最佳化偏好差距。",
      useWhen: "一般對齊任務、資源有限、想要穩定且容易重現的訓練流程。",
      avoidWhen: "偏好標註雜訊高、品質不一致，或需要 PPO 等級的細粒度 reward 控制。",
      notes: "DPO 對資料品質敏感；先清理衝突與重複 pair，再調 beta 與學習率。",
    },
    en: {
      summary: "Skips the reward model and RL, optimizing the preference gap directly on chosen/rejected pairs.",
      useWhen: "Use for general alignment, limited resources, and a stable, reproducible training flow.",
      avoidWhen: "Avoid with noisy or inconsistent preference labels, or when PPO-level fine reward control is required.",
      notes: "DPO is sensitive to data quality; clean conflicting and duplicate pairs before tuning beta and learning rate.",
    },
  },
  {
    id: "grpo",
    ...
    course: "Class7",
    page: "content/Tool_wiki/tools/class7-alignment.md",
    tags: ["grpo", "group-relative", "reasoning", "alignment", "sample-efficient"],
    itemType: "Method",
    role: "Group-relative advantage without a separate reward model",
    abstractionLevel: "Alignment training method",
    learningLevel: "Advanced",
    stack: "Class7 alignment stack",
    learnerNote: {
      zh: "GRPO 對每個 prompt 取一組 sample，用組內 relative advantage 對齊，常用於推理任務。",
      en: "GRPO samples a group per prompt and aligns on within-group relative advantage, often for reasoning tasks.",
    },
    zh: {
      summary: "不訓練獨立 reward model，改用一組 sample 的相對優勢來更新 policy。",
      useWhen: "推理或複雜任務、想要取樣效率與彈性、可接受每 prompt 多次取樣的成本。",
      avoidWhen: "任務簡單、DPO 已足夠，或無法負擔多 sample 的生成成本。",
      notes: "每 prompt 要多次取樣，計算不一定比較省；先確認 reward/score 訊號是否可靠。",
    },
    en: {
      summary: "Skips a separate reward model and updates the policy using the relative advantage within a group of samples.",
      useWhen: "Use for reasoning or complex tasks needing sample efficiency and flexibility, when multi-sample cost is acceptable.",
      avoidWhen: "Avoid for simple tasks where DPO suffices, or when multi-sample generation cost is too high.",
      notes: "It samples multiple generations per prompt, so it is not always cheaper; confirm the reward/score signal is reliable.",
    },
  },
  {
    id: "preference-dataset",
    name: "Preference dataset (chosen/rejected)",
    category: "Alignment data",
    stage: "Alignment",
    status: "Course-covered",
    course: "Class7",
    page: "content/Tool_wiki/tools/class7-alignment.md",
    tags: ["preference", "annotation", "chosen-rejected", "gradio", "alignment"],
    itemType: "Pattern",
    role: "Provide the chosen/rejected training signal for alignment",
    abstractionLevel: "Alignment data pattern",
    learningLevel: "Intermediate",
    stack: "Class7 alignment stack",
    learnerNote: {
      zh: "偏好資料是對齊的訓練訊號；每筆是 prompt + chosen + rejected，可用 Gradio 介面人工標註。",
      en: "Preference data is the alignment signal; each record is prompt + chosen + rejected, annotatable via a Gradio interface.",
    },
    zh: {
      summary: "收集 prompt 對應的較佳與較差回答，組成 chosen/rejected pair 作為對齊訓練資料。",
      useWhen: "要跑 PPO/DPO/GRPO，需要一致、可追蹤來源的偏好標註。",
      avoidWhen: "標註標準不一致、標註者沒有共識，或 pair 之間差異不明確。",
      notes: "固定標註準則並抽樣複檢；衝突與重複 pair 會直接拉低對齊品質。",
    },
    en: {
      summary: "Collects better and worse answers per prompt into chosen/rejected pairs as alignment training data.",
      useWhen: "Use when running PPO/DPO/GRPO and needing consistent, source-traceable preference labels.",
      avoidWhen: "Avoid with inconsistent standards, no annotator consensus, or unclear differences between pairs.",
      notes: "Fix a labeling rubric and sample-review; conflicting and duplicate pairs directly lower alignment quality.",
    },
  },
  {
    id: "reward-model",
    name: "Reward model",
    category: "Alignment component",
    stage: "Alignment",
    status: "Course-covered",
    course: "Class7",
    page: "content/Tool_wiki/tools/class7-alignment.md",
    tags: ["reward-model", "ppo", "rlhf", "scoring", "alignment"],
    itemType: "Method",
    role: "Turn preferences into a differentiable reward for PPO",
    abstractionLevel: "Alignment training component",
    learningLevel: "Advanced",
    stack: "Class7 alignment stack",
    learnerNote: {
      zh: "Reward model 是 PPO 路線的中介：把偏好轉成可微分 reward，但容易被 reward hacking。",
      en: "The reward model is PPO's intermediary: it turns preferences into a differentiable reward but is prone to reward hacking.",
    },
    zh: {
      summary: "用偏好資料訓練一個對回答打分的模型，作為 PPO 的 reward signal 來源。",
      useWhen: "走 PPO 路線、需要可微分 reward 並對行為做細控時。",
      avoidWhen: "用 DPO/GRPO 等無 reward model 方法，或偏好資料不足以訓出可靠 reward。",
      notes: "監控 reward 分佈與 over-optimization；高 reward 不等於真實品質，要靠 eval 校正。",
    },
    en: {
      summary: "Trains a model to score answers from preference data, providing the reward signal for PPO.",
      useWhen: "Use on the PPO route when you need a differentiable reward and fine behavior control.",
      avoidWhen: "Avoid with reward-model-free methods (DPO/GRPO) or when preference data cannot yield a reliable reward.",
      notes: "Monitor reward distribution and over-optimization; high reward is not real quality—correct it with evals.",
    },
  },
  {
    id: "trl-alignment-trainers",
    name: "TRL alignment trainers (DPO/PPO/GRPO)",
    category: "Alignment library",
    stage: "Alignment",
    status: "Course-covered",
    course: "Class7",
    page: "content/Tool_wiki/tools/class7-alignment.md",
    tags: ["trl", "dpo-trainer", "ppo-trainer", "grpo-trainer", "alignment"],
    itemType: "Library",
    role: "Wrap PPO/DPO/GRPO into reproducible training flows",
    abstractionLevel: "Alignment training library",
    learningLevel: "Intermediate",
    stack: "Class7 alignment stack",
    learnerNote: {
      zh: "TRL 提供 DPOTrainer/PPOTrainer/GRPOTrainer，把三種對齊方法包成一致的訓練介面。",
      en: "TRL provides DPOTrainer/PPOTrainer/GRPOTrainer, wrapping the three alignment methods in a consistent interface.",
    },
    zh: {
      summary: "用 TRL 的 trainer 在 Hugging Face 模型上跑 DPO/PPO/GRPO，搭配 PEFT 做參數高效對齊。",
      useWhen: "要在現有 HF 生態快速實作並比較三種對齊方法，而不想自己手刻訓練迴圈。",
      avoidWhen: "需要客製 loss 或非標準對齊流程，框架抽象反而綁手綁腳時。",
      notes: "對齊與 Class5 的 SFTTrainer 同屬 TRL，但用的是 preference data 與不同 loss。",
    },
    en: {
      summary: "Uses TRL trainers to run DPO/PPO/GRPO on Hugging Face models, with PEFT for parameter-efficient alignment.",
      useWhen: "Use to implement and compare the three methods quickly in the HF ecosystem without hand-writing a loop.",
      avoidWhen: "Avoid when a custom loss or non-standard flow makes the framework abstraction restrictive.",
      notes: "Alignment shares TRL with Class5's SFTTrainer, but uses preference data and different losses.",
    },
  },
  {
    id: "ai-safety-pipeline",
    name: "AI safety testing and guardrails",
    category: "Safety workflow",
    stage: "Safety/Guardrails",
    status: "Course-covered",
    course: "Class8",
    page: "content/Tool_wiki/tools/class8-ai-safety.md",
    tags: ["ai-safety", "guardrails", "hallucination", "jailbreak", "bias"],
    itemType: "Workflow stack",
    role: "Test LLM failures and add pre/post request protection",
    abstractionLevel: "Workflow stack: detectors plus safety wrapper",
    learningLevel: "Intermediate",
    stack: "Class8 AI safety stack",
    learnerNote: {
      zh: "Class8 用真實 API 觀察幻覺/越獄/偏見，再把偵測與防護串成 request 前後的 safety pipeline。",
      en: "Class8 observes hallucination/jailbreak/bias on real APIs, then wires detection and defense into a pre/post safety pipeline.",
    },
    zh: {
      summary: "把幻覺、越獄、偏見的偵測組成 safety wrapper，並用測試集量化封鎖率與偵測率。",
      useWhen: "要把 LLM 放進產品，需要在呼叫前後加上防護層並量化失敗率。",
      avoidWhen: "只是內部實驗、沒有外部使用者，或防護需求尚未定義。",
      notes: "保留每次 request 的風險分數、違規類別與 log，定期檢查防護是否退步。",
    },
    en: {
      summary: "Composes hallucination, jailbreak, and bias detection into a safety wrapper, quantifying block and detection rates with test sets.",
      useWhen: "Use when putting an LLM into a product that needs protection around calls and measurable failure rates.",
      avoidWhen: "Avoid for internal-only experiments with no external users or undefined protection needs.",
      notes: "Keep per-request risk scores, violation types, and logs, and review whether protection regresses.",
    },
  },
  {
    ...
    course: "Class8",
    page: "content/Tool_wiki/tools/class8-ai-safety.md",
    tags: ["hallucination", "factual-verification", "citation", "confidence", "safety"],
    itemType: "Pattern",
    role: "Detect confidently stated false information",
    abstractionLevel: "Safety detection pattern",
    learningLevel: "Intermediate",
    stack: "Class8 AI safety stack",
    learnerNote: {
      zh: "用 factual verification、citation 驗證與 confidence scoring 抓出模型自信講錯的內容。",
      en: "Uses factual verification, citation validation, and confidence scoring to catch confidently false content.",
    },
    zh: {
      summary: "以規則與已知事實比對、來源驗證與不確定性指標，標記可能的幻覺回答。",
      useWhen: "輸出涉及事實、引用或統計，且錯誤資訊會造成風險時。",
      avoidWhen: "純創意/主觀內容，或事實正確性不是關鍵的場景。",
      notes: "規則型偵測有 false positive/negative；高風險場景仍需外部知識庫或人工查核。",
    },
    en: {
      summary: "Flags likely hallucinations using rule/known-fact matching, source validation, and uncertainty indicators.",
      useWhen: "Use when outputs involve facts, citations, or statistics and false info creates risk.",
      avoidWhen: "Avoid for purely creative/subjective content where factual accuracy is not critical.",
      notes: "Rule-based detection has false positives/negatives; high-risk cases still need a knowledge base or human review.",
    },
  },
  {
    id: "jailbreak-red-teaming",
    name: "Jailbreak red-teaming",
    category: "Safety pattern",
    stage: "Safety/Guardrails",
    status: "Course-covered",
    course: "Class8",
    page: "content/Tool_wiki/tools/class8-ai-safety.md",
    tags: ["jailbreak", "red-teaming", "prompt-injection", "dan", "safety"],
    itemType: "Pattern",
    role: "Test and detect attempts to bypass safety limits",
    abstractionLevel: "Safety testing pattern",
    learningLevel: "Intermediate",
    stack: "Class8 AI safety stack",
    learnerNote: {
      zh: "用已知 jailbreak pattern（DAN、roleplay、翻譯繞道）測防護，並偵測繞過嘗試；僅供防禦理解。",
      en: "Tests defenses with known jailbreak patterns (DAN, roleplay, translation bypass) and detects attempts; for defensive understanding only.",
    },
    zh: {
      summary: "用結構化的 jailbreak 測試集衡量防護的封鎖率與偵測率，並辨識常見繞過手法。",
      useWhen: "要評估與強化 LLM 應用的安全防線，理解攻擊面向以建立防禦。",
      avoidWhen: "缺乏授權或明確防禦目的，或會被用於實際攻擊時。",
      notes: "「沒觸發偵測」不代表安全，可能只是測試集沒覆蓋；攻擊手法會持續演進。",
    },
    en: {
      summary: "Uses structured jailbreak test sets to measure block/detection rates and identify common bypass techniques.",
      useWhen: "Use to assess and harden an LLM app's defenses and understand the attack surface for defense.",
      avoidWhen: "Avoid without authorization or a clear defensive purpose, or where it could enable real attacks.",
      notes: "No detection does not mean safe—the set may not cover it; techniques keep evolving.",
    },
  },
  {
    id: "bias-fairness-detection",
    name: "Bias and fairness detection",
    category: "Safety pattern",
    stage: "Safety/Guardrails",
    status: "Course-covered",
    course: "Class8",
    page: "content/Tool_wiki/tools/class8-ai-safety.md",
    tags: ["bias", "fairness", "ethics", "fairlearn", "aif360"],
    itemType: "Pattern",
    role: "Detect gender, cultural, and demographic bias",
    abstractionLevel: "Safety analysis pattern",
    learningLevel: "Intermediate",
    stack: "Class8 AI safety stack",
    learnerNote: {
      zh: "用 bias scoring 與 fairness 工具（fairlearn/AIF360）分析輸出中的刻板印象與族群偏差。",
      en: "Uses bias scoring and fairness tools (fairlearn/AIF360) to analyze stereotyping and demographic bias in outputs.",
    },
    zh: {
      summary: "檢測性別、文化、族群刻板印象，對輸出做 bias 分數與敏感度分析。",
      useWhen: "輸出會影響人、涉及招聘/評價/建議等對公平性敏感的場景。",
      avoidWhen: "把 bias 分數當成唯一判準，忽略情境與人工判讀時。",
      notes: "偵測工具本身也有偏差與侷限；分數要搭配情境理解與人工複核。",
    },
    en: {
      summary: "Detects gender, cultural, and demographic stereotyping with bias scores and sensitivity analysis on outputs.",
      useWhen: "Use when outputs affect people or involve fairness-sensitive cases like hiring, evaluation, or recommendations.",
      avoidWhen: "Avoid treating bias scores as the sole criterion while ignoring context and human judgment.",
      notes: "Detection tools have their own biases and limits; pair scores with context and human review.",
    },
  },
  {
    id: "safety-guardrail-wrapper",
    name: "Safety guardrail wrapper (pre/post filter)",
    category: "Safety component",
    stage: "Safety/Guardrails",
    status: "Course-covered",
    course: "Class8",
    page: "content/Tool_wiki/tools/class8-ai-safety.md",
    tags: ["guardrails", "safety-filter", "pre-post-filter", "logging", "safety"],
    itemType: "Pattern",
    role: "Intercept prompts and responses to block, rewrite, or flag",
    abstractionLevel: "Safety implementation pattern",
    learningLevel: "Intermediate",
    stack: "Class8 AI safety stack",
    learnerNote: {
      zh: "Safety wrapper 在 request 前後攔截 prompt 與 response，回傳結構化的安全 metadata。",
      en: "A safety wrapper intercepts prompts and responses around a request and returns structured safety metadata.",
    },
    zh: {
      summary: "把多個偵測器組成 request 前/後的防護層，輸出風險分數、違規類別與處置決定。",
      useWhen: "要把安全檢查標準化、可記錄、可監控地包進 LLM 呼叫流程。",
      avoidWhen: "只是 demo、不需要持續監控，或防護規則尚未定義時。",
      notes: "搭配 logging 與統計，定期回看 block/detection rate；防護是持續演進，不是一次性設定。",
    },
    en: {
      summary: "Composes detectors into pre/post protection around a request, emitting risk score, violation type, and an action decision.",
      useWhen: "Use to wrap safety checks into the call flow in a standardized, loggable, monitorable way.",
      avoidWhen: "Avoid for demos with no monitoring need or when protection rules are undefined.",
      notes: "Pair with logging and stats, review block/detection rates regularly; protection evolves, it is not set-once.",
    },
  },
  {
    id: "content-moderation",
    name: "Content moderation and sanitization",
    category: "Safety component",
    stage: "Safety/Guardrails",
    status: "Course-covered",
    course: "Class8",
    page: "content/Tool_wiki/tools/class8-ai-safety.md",
    tags: ["moderation", "sanitization", "input-filtering", "bleach", "safety"],
    itemType: "Method",
    role: "First-layer input/output filtering before deeper checks",
    abstractionLevel: "Safety filtering method",
    learningLevel: "Beginner",
    stack: "Class8 AI safety stack",
    learnerNote: {
      zh: "Moderation 與輸入淨化（如 bleach）是第一道過濾，攔掉明顯有害內容，再交給更細的偵測器。",
      en: "Moderation and input sanitization (e.g. bleach) are the first filter, catching obvious harm before deeper detectors.",
    },
    zh: {
      summary: "用 moderation API 與輸入淨化，先擋下明顯違規或有害內容作為第一層防護。",
      useWhen: "需要快速、低成本地過濾明顯有害輸入/輸出，作為防護的最外層。",
      avoidWhen: "把 moderation 當成完整 safety，忽略 application 層的 pre/post 檢查與人工審核。",
      notes: "moderation 只是其中一層；複雜攻擊與細微 bias 仍需專門偵測器與人工判讀。",
    },
    en: {
      summary: "Uses a moderation API and input sanitization to block obviously harmful content as the outer protection layer.",
      useWhen: "Use to filter clearly harmful inputs/outputs quickly and cheaply as the outermost layer.",
      avoidWhen: "Avoid treating moderation as full safety while skipping app-layer pre/post checks and human review.",
      notes: "Moderation is one layer; complex attacks and subtle bias still need dedicated detectors and human review.",
    },
  },
  {
    id: "sft",
...
export const pipelineHighlights = [
  ...
  {
    stage: "Alignment",
    zh: "從 preference data 走到 PPO、DPO、GRPO 的並排比較、reward model 與 TRL alignment trainers。",
    en: "From preference data to a side-by-side comparison of PPO, DPO, and GRPO, reward models, and TRL alignment trainers.",
  },
  {
    stage: "Safety/Guardrails",
    zh: "從幻覺、jailbreak、bias 的實測走到 pre/post safety wrapper、moderation 與量化的 block/detection rate。",
    en: "From hands-on hallucination, jailbreak, and bias testing to pre/post safety wrappers, moderation, and quantified block/detection rates.",
  },
];
